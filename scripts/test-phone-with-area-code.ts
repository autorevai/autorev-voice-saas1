...
async function testPhoneWithAreaCode() {
  const vapi = new VapiClient({ token: process.env.VAPI_API_KEY! })
  
  try {
    console.log('🧪 Testing Phone Number Creation with Area Code...')
    console.log('=' .repeat(50))
    
    // Use the assistant we created earlier
    const assistantId = '6d1baaae-37b8-4741-9d5b-6773c3b8f12f'
    
    console.log('📞 Creating phone number with area code 740...')

...
async function test5CompleteFlow() {
  console.log('🧪 Testing Complete Flow with test5@autorev.ai')
  console.log('==================================================')
  ...
  try {
    ...
    // Step 7: Save assistant to database with webhook URL
    console.log('💾 Step 7: Saving assistant to database with webhook URL...')
    ...
    console.log(`   ✅ Assistant saved with webhook URL: ${webhookUrl}`)
    ...
    const { data: finalAssistant } = await db
      .from('assistants')
      .select('vapi_assistant_id, vapi_number_id, webhook_url')
    ...
    console.log(`   ✅ Webhook URL: ${finalAssistant?.webhook_url}`)
    
    // Step 10: Create 5 test booking calls
    console.log('📞 Step 10: Creating 5 test booking calls...')
    const testCalls = []
    ...
    // Step 11: Test dashboard metrics
    console.log('📊 Step 11: Testing dashboard metrics...')
    ...
    // Step 12: Test webhook URL relationship
    console.log('🔗 Step 12: Testing webhook URL relationship...')
    const { data: assistantWithWebhook } = await db
      .from('assistants')
      .select(`
        id,
        vapi_assistant_id,
    ...
    if (assistantWithWebhook) {
      console.log(`   ✅ Webhook URL: ${assistantWithWebhook.webhook_url}`)
      console.log(`   ✅ Tenant: ${(assistantWithWebhook.tenants as any).name}`)
      console.log(`   ✅ User: ${(assistantWithWebhook.users as any).email}`)
    }
    ...
    console.log('🔗 Database relationships verified: User → Tenant → Assistant → Webhook URL')

...
async function testCompleteProductionFlow() {
  ...
  try {
    console.log('🚀 Testing Complete Production Flow Like Smith AI...')
    console.log('=' .repeat(70))
    
    // Step 1: Create a new user (simulate signup)
    console.log('👤 Step 1: Create New User (Signup)')
    const testUserId = '00000000-0000-0000-0000-' + Date.now().toString().slice(-12).padStart(12, '0')
    const testEmail = `production-test-${Date.now()}@autorev.ai`
    const businessName = `Production Test HVAC ${Date.now()}`
    ...
    // Step 3: Test dashboard data fetch (simulate what dashboard does)
    console.log('')
    console.log('📊 Step 3: Test Dashboard Data Fetch')
    try {
      // This simulates the exact dashboard logic
      ...
      if (userRecordError) {
        console.error('❌ Dashboard user fetch failed:', userRecordError)
        return
      }
      ...
      // Check for assistant (should not exist yet)
      ...
      if (assistantError) {
        ...
        console.log(`   ✅ VAPI Phone: ${assistant.vapi_number_id || 'NULL'}`)
      } else {
    ...
    // Step 4: Test VAPI provisioning (simulate setup wizard)
    console.log('')
    console.log('🤖 Step 4: Test VAPI Provisioning (Setup Wizard)')
    ...
    try {
      ...
      if (result.success) {
        ...
        // Save assistant to database (simulate what API does)
    ...
    // Step 5: Test final dashboard state
    console.log('')
    console.log('🎯 Step 5: Test Final Dashboard State')
    ...
    // Step 6: Test dashboard display logic
    console.log('')
    console.log('📱 Step 6: Test Dashboard Display Logic')
    ...
    console.log('   Complete end-to-end flow working perfectly')
